...
img_1 = np.asarray(Image.open("resources/tsukuba_1.png"))
img_2 = np.asarray(Image.open("resources/tsukuba_2.png"))
img_3 = np.asarray(Image.open("resources/tsukuba_3.png"))
img_4 = np.asarray(Image.open("resources/tsukuba_4.png"))
depth = np.asarray(Image.open("resources/tsukuba_depth.png"))


@pytest.mark.parametrize("left_right_translation", [(img_1, img_2, 13),
                                                    (img_1, img_3, 22),
                                                    (img_1, img_4, 33),
                                                    (img_3, img_4, 14)])
def test_max_translation(left_right_translation):
    left, right, translation = left_right_translation
    assert get_max_translation(left, right) == translation
...
def test_disparity_hypothesis(left_right_disparity_pad_sum):
    ...
    assert all_similar(render_disparity_hypothesis(left, right, disparity, pad).sum(), sum)
...
def test_disparity_as_segmentation(left_right_depth_translation_threshold):
    ...
    dm_filtered = disparity_map(left,
                                 right,
...
@pytest.mark.parametrize("left_right_translation_ssim_mse", [(img_1, img_2, -15, .55, .006),
                                                                (img_2, img_3, -18, .55, .006),
                                                                (img_3, img_4, -18, .55, .0053)])
